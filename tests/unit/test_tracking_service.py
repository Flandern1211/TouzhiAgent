...
def test_tracking_source_status_exposes_crawler_path_and_fallback_reason():
    repository = InMemoryRepository()
    source = StaticSource(
        SourceType.NEWS,
        [Evidence(
            ...
            metadata={
                "crawler": "internal",
                "fallback": True,
                "external_failure_reason": "external_timeout",
            },
        )],
    )
    service = TrackingService(
    ...
    status = result.source_statuses["news"]
    assert status.crawler == "internal"
    assert status.fallback is True
    assert status.failure_reason == "external_timeout"
