...
def test_settings_use_safe_defaults_for_local_service_and_schedule():
    ...
    assert settings.crawler_timeout_seconds == 10.0
    assert settings.crawler_max_retries == 2
    assert settings.crawler_max_response_bytes == 2_000_000
    assert settings.crawler_min_interval_seconds == 0.25
    assert settings.crawler_allowed_domains == ()
    assert settings.crawler_follow_redirects is True
...
def test_settings_read_database_url_crawler_and_risk_thresholds():
    settings = Settings.from_env(
        {
            ...
            "FUND_AGENT_CRAWLER_ALLOWED_DOMAINS": "fund.test, *.news.test",
            "FUND_AGENT_CRAWLER_TIMEOUT_SECONDS": "12.5",
            "FUND_AGENT_CRAWLER_MAX_RETRIES": "4",
            "FUND_AGENT_CRAWLER_MAX_RESPONSE_BYTES": "4096",
            "FUND_AGENT_CRAWLER_MIN_INTERVAL_SECONDS": "1.5",
            "FUND_AGENT_CRAWLER_USER_AGENT": "TestCrawler/1.0",
            "FUND_AGENT_CRAWLER_FOLLOW_REDIRECTS": "false",
            "FUND_AGENT_CRAWLER_RESPECT_ROBOTS": "false",
    ...
    assert settings.crawler_allowed_domains == ("fund.test", "*.news.test")
    assert settings.crawler_timeout_seconds == 12.5
    assert settings.crawler_max_retries == 4
    assert settings.crawler_max_response_bytes == 4096
    assert settings.crawler_min_interval_seconds == 1.5
    assert settings.crawler_user_agent == "TestCrawler/1.0"
    assert settings.crawler_follow_redirects is False
    assert settings.crawler_respect_robots is False
...
def test_settings_safe_repr_and_model_dump_do_not_expose_secrets():
    ...
    assert "mysql_host" not in settings.model_dump()
    assert "mysql_port" not in settings.model_dump()
    assert "mysql_database" not in settings.model_dump()
    assert "mysql_user" not in settings.model_dump()
