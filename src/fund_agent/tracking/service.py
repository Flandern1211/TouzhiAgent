...
class TrackingService:
    ...
    def _collect_source(
        ...
        return SourceStatus(
            ...
            crawler=next((str(record.metadata["crawler"]) for record in source_records if record.metadata.get("crawler")), None),
            fallback=any(bool(record.metadata.get("fallback")) for record in source_records),
            failure_reason=next(
                (
                    str(record.metadata.get("failure_reason") or record.metadata["external_failure_reason"])
                    for record in source_records
                    if record.metadata.get("failure_reason") or record.metadata.get("external_failure_reason")
                ),
                None,
            ),
        )
