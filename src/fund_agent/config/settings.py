...
class Settings(BaseModel):
    ...
    def model_dump(self, *, include_secrets: bool = False, **kwargs: Any) -> dict[str, Any]:
        ...
        if include_secrets:
            return dumped
        for field_name in ("database_url", "mysql_password", "crawler_api_key"):
    ...
    def model_dump_json(self, *, include_secrets: bool = False, **kwargs: Any) -> str:
        ...
        if isinstance(excluded, dict):
            excluded.update({"database_url": True, "mysql_password": True, "crawler_api_key": True})
        else:
            excluded.update({"database_url", "mysql_password", "crawler_api_key"})
